<!-- casework-lane {"claim_id":"C01","elements":["C01.E5","C02.E2","C04.E3","C07.E3"],"materializer":2,"outputs":["research/grok-out/MD52-frontier-gap-generation.csv"],"pack":"metr_deep","plan_lane":"MD52","plan_sha256":"7ff1e5a3d68d5eadfdb76c0ff002060e21ce4adbff1fa3c5c6ef52d5cad0f9e2"} -->

# MD52 — Frontier-driven gap generation
...
Scope: Frontier-driven gap generation.
Primary routes: the engine frontier and open elements.
...
Row typing: put the target table prefix in target_prefix on every row (MDE entity, MDF funding event, MDI in-kind or access, MDP provenance proposition, MDQ project conflict, MDR relationship, MDS source coverage and bounded negative, MDT timeline event) and put the case element ids this row would support in element_ids, semicolon separated, from: C01.E5, C02.E2, C04.E3, C07.E3. Leave any column that does not apply to a row empty rather than guessing.

Task A — Run the engine frontier and list every untouched source class and every open element with its claim.
Task B — For each, decide whether a currently available public route exists, and record the route or the reason none exists.
Task C — Write one targeted successor brief in the MD70 to MD89 range for each real gap, citing the finding that created it, and create none for an element that merely lacks detail.
Task D — Record every class that is not applicable with its reason, and every future document with its next check date.
