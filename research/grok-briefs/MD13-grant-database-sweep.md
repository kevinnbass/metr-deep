<!-- casework-lane {"claim_id":"C01","elements":["C01.E5","C02.E2"],"materializer":2,"outputs":["research/grok-out/MD13-grant-database-sweep.csv"],"pack":"metr_deep","plan_lane":"MD13","plan_sha256":"7ff1e5a3d68d5eadfdb76c0ff002060e21ce4adbff1fa3c5c6ef52d5cad0f9e2"} -->

# MD13 — Universal public grant-database sweep
...
Scope: Universal public grant-database sweep.
Primary routes: official grantmaker databases enumerated by canonical page, sitemap or API; Candid used only to locate a primary.
...
Row typing: put the target table prefix in target_prefix on every row (MDE entity, MDF funding event, MDI in-kind or access, MDP provenance proposition, MDQ project conflict, MDR relationship, MDS source coverage and bounded negative, MDT timeline event) and put the case element ids this row would support in element_ids, semicolon separated, from: C01.E5, C02.E2. Leave any column that does not apply to a row empty rather than guessing.

Task A — Build the alias list first: METR, Model Evaluation and Threat Research, METR Inc, ARC Evals, Alignment Research Center Evals, and the EIN, and record it as a row so later lanes reuse it.
Task B — For each named supporter's grant database and for the major independent AI-safety grant databases, enumerate the canonical index, sitemap or API rather than relying on keyword search alone, and record the enumeration route, the record count seen and any cap hit.
Task C — Record every award found with payer, recipient alias matched, amount, date, purpose and the canonical record URL.
Task D — Record a bounded negative for each database enumerated with no METR record, naming the enumeration route and the record count, so the Packard-style miss cannot recur.
