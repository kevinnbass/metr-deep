<!-- casework-lane {"claim_id":"C01","elements":["C01.E2","C01.E3","C01.E4"],"materializer":2,"outputs":["research/grok-out/MD46-funding-reconciliation.csv"],"pack":"metr_deep","plan_lane":"MD46","plan_sha256":"7ff1e5a3d68d5eadfdb76c0ff002060e21ce4adbff1fa3c5c6ef52d5cad0f9e2"} -->

# MD46 — Full typed funding reconciliation
...
Scope: Full typed funding reconciliation.
Primary routes: promoted rows only.
...
Row typing: put the target table prefix in target_prefix on every row (MDE entity, MDF funding event, MDI in-kind or access, MDP provenance proposition, MDQ project conflict, MDR relationship, MDS source coverage and bounded negative, MDT timeline event) and put the case element ids this row would support in element_ids, semicolon separated, from: C01.E2, C01.E3, C01.E4. Leave any column that does not apply to a row empty rather than guessing.

Task A — Rebuild the reconciliation from promoted rows only: the denominator, each compatible component with the reason it is compatible, and each excluded amount with the reason it is not.
Task B — Record the identified compatible total and the unresolved remainder or range as arithmetic over those rows.
Task C — Record every money type present and confirm that no two types, periods or currencies were summed.
Task D — List the exact residual cells that remain unknown after the reconciliation, in the form a later request would need.
