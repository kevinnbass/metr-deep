<!-- casework-lane {"claim_id":"C01","elements":["C01.E1","C01.E2","C01.E3","C01.E4"],"materializer":2,"outputs":["research/grok-out/MD14-six-month-chronology.csv"],"pack":"metr_deep","plan_lane":"MD14","plan_sha256":"7ff1e5a3d68d5eadfdb76c0ff002060e21ce4adbff1fa3c5c6ef52d5cad0f9e2"} -->

# MD14 — Six-month window chronology and denominator compatibility
...
Scope: Six-month window chronology and denominator compatibility.
Primary routes: METR blog and archives; all Wave 1 rows; fiscal and payment dates.
...
Row typing: put the target table prefix in target_prefix on every row (MDE entity, MDF funding event, MDI in-kind or access, MDP provenance proposition, MDQ project conflict, MDR relationship, MDS source coverage and bounded negative, MDT timeline event) and put the case element ids this row would support in element_ids, semicolon separated, from: C01.E1, C01.E2, C01.E3, C01.E4. Leave any column that does not apply to a row empty rather than guessing.

Task A — Capture METR's own statement of the approximately $71 million total with its URL, verbatim sentence, the period it names, the response hash and the UTC check time, and record the money type it uses.
Task B — Build the dated chronology of every public funding event between the window's start and end, one row per event, with money type and payment status.
Task C — For each known amount, record whether it is compatible with the denominator's period, entity and money type, and give the reason for every exclusion.
Task D — State the identified compatible total and the unresolved remainder or range arithmetically from the rows, with no imputed allocation and no forced equality.
