<!-- casework-lane {"claim_id":"C10","elements":["C10.E1","C10.E2","C10.E3"],"materializer":2,"outputs":["research/grok-out/MD49-claim-adjudication.csv"],"pack":"metr_deep","plan_lane":"MD49","plan_sha256":"7ff1e5a3d68d5eadfdb76c0ff002060e21ce4adbff1fa3c5c6ef52d5cad0f9e2"} -->

# MD49 — Claim-by-claim adjudication of the post and the response
...
Scope: Claim-by-claim adjudication of the post and the response.
Primary routes: the frozen public clone; the public response; promoted rows.
...
Row typing: put the target table prefix in target_prefix on every row (MDE entity, MDF funding event, MDI in-kind or access, MDP provenance proposition, MDQ project conflict, MDR relationship, MDS source coverage and bounded negative, MDT timeline event) and put the case element ids this row would support in element_ids, semicolon separated, from: C10.E1, C10.E2, C10.E3. Leave any column that does not apply to a row empty rather than guessing.

Task A — Record the posted version and the current version of each disputed figure separately, each with its commit and hash, and never let a later correction stand in for what was posted.
Task B — Adjudicate each sentence of the public response against the promoted rows: accurate, incomplete, incorrect or undetermined, with the row ids and the reason.
Task C — Adjudicate each claim of the underlying post the same way.
Task D — For every verdict short of accurate, name the exact document that would settle it.
