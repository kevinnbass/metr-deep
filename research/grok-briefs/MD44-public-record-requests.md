<!-- casework-lane {"claim_id":"C06","elements":["C06.E3"],"materializer":2,"outputs":["research/grok-out/MD44-public-record-requests.csv"],"pack":"metr_deep","plan_lane":"MD44","plan_sha256":"7ff1e5a3d68d5eadfdb76c0ff002060e21ce4adbff1fa3c5c6ef52d5cad0f9e2"} -->

# MD44 — Public-record request drafts for contract amounts and terms
...
Scope: Public-record request drafts for contract amounts and terms.
Primary routes: UK freedom-of-information; US freedom-of-information; Commission access to documents.
...
Task A — Record, for each unresolved contract element, the exact authority that holds the record and the statutory route that would produce it, with the citation and the response clock.
Task B — Confirm first that the document is not already published, recording the publication routes checked.
Task C — Draft each request as a narrow ask for a specific existing record, never as a question for an explanation and never as any form of allocation question, and save it with approved_for_send false.
Task D — Record each draft in the requests journal with its state, and send nothing.
...
Output `research/grok-out/MD44-public-record-requests.csv` with columns: `row,task,target_prefix,element_ids,subject,entity_type,aliases,identifier,from_entity,intermediary,to_entity,relationship_type,start_date,end_date,ledger,money_type,amount_usd,currency,date,date_precision,period,payment_status,purpose_restriction,project,provider,in_kind_type,quantity_or_value,terms,policy_version,disclosure_recusal,provider_authority,personnel_conflicts,edge_type,documented_transaction,source_class,query_or_endpoint,url,quote_300,checked_utc,result,result_count,next_document,limitation,note`.

Standard goal:

```text
/goal Run research/grok-briefs/MD44-public-record-requests.md exactly as written; write only research/grok-out/MD44-public-record-requests.csv and research/primary/MD44-public-record-requests/
```
