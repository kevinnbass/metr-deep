<!-- casework-lane {"claim_id":"C08","elements":["C08.E5"],"materializer":2,"outputs":["research/grok-out/MD35-evaluator-standards.csv"],"pack":"metr_deep","plan_lane":"MD35","plan_sha256":"7ff1e5a3d68d5eadfdb76c0ff002060e21ce4adbff1fa3c5c6ef52d5cad0f9e2"} -->

# MD35 — External-evaluator independence standards compared with METR practice
...
Scope: External-evaluator independence standards compared with METR practice.
Primary routes: AEF-1; government evaluator standards; other published evaluator policies.
...
Row typing: put the target table prefix in target_prefix on every row (MDE entity, MDF funding event, MDI in-kind or access, MDP provenance proposition, MDQ project conflict, MDR relationship, MDS source coverage and bounded negative, MDT timeline event) and put the case element ids this row would support in element_ids, semicolon separated, from: C08.E5. Leave any column that does not apply to a row empty rather than guessing.

Task A — Record the requirements each published external-evaluator standard states, quoted verbatim, with its publisher and date.
Task B — Record METR's own self-assessment answers against those requirements, quoted verbatim with the date.
Task C — Record other evaluators' published independence policies for comparison, with source and date.
Task D — Record for each requirement whether the public record shows METR meeting it, not meeting it, or leaves it undetermined, and which document would determine it.
