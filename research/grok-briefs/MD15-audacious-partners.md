<!-- casework-lane {"claim_id":"C06","elements":["C06.E2","C09.E1"],"materializer":2,"outputs":["research/grok-out/MD15-audacious-partners.csv"],"pack":"metr_deep","plan_lane":"MD15","plan_sha256":"7ff1e5a3d68d5eadfdb76c0ff002060e21ce4adbff1fa3c5c6ef52d5cad0f9e2"} -->

# MD15 — Audacious partner list by date and project-funder rules

Plan directive:

Slice S2, wave 2. This lane belongs to /mnt/f/projects/anthropic/metr_deep/PLAN.md; its id, slug and scope are frozen and are never renumbered or repurposed.

Scope: Audacious partner list by date and project-funder rules.
Primary routes: Audacious Project and TED live pages plus Wayback; partner descriptions.
...
Row typing: put the target table prefix in target_prefix on every row (MDE entity, MDF funding event, MDI in-kind or access, MDP provenance proposition, MDQ project conflict, MDR relationship, MDS source coverage and bounded negative, MDT timeline event) and put the case element ids this row would support in element_ids, semicolon separated, from: C06.E2, C09.E1. Leave any column that does not apply to a row empty rather than guessing.

Task A — Capture the Audacious Project partner list as it stood on the Canary announcement date and as it stands now, each from a dated archive or live capture with a hash, and record every name added or removed between them.
Task B — Record what the Audacious Project's own materials say about how partners fund projects: whether partners pay grantees directly, whether membership implies participation in a given project, and the exact wording.
Task C — Record for each partner whether any public document ties it to the Canary project specifically, and mark every partner with no such document as membership only.
Task D — Record explicitly that a partner's appearance on a current list is not evidence of the date it joined, and date each listing change from the archive capture instead.
