<!-- casework-lane {"claim_id":"C08","elements":["C08.E5","C06.E3"],"materializer":2,"outputs":["research/grok-out/MD36-evaluator-selection.csv"],"pack":"metr_deep","plan_lane":"MD36","plan_sha256":"7ff1e5a3d68d5eadfdb76c0ff002060e21ce4adbff1fa3c5c6ef52d5cad0f9e2"} -->

# MD36 — Evaluator-selection market and who selected METR for each role
...
Scope: Evaluator-selection market and who selected METR for each role.
Primary routes: procurement and request-for-information records; company and government documents.
...
Row typing: put the target table prefix in target_prefix on every row (MDE entity, MDF funding event, MDI in-kind or access, MDP provenance proposition, MDQ project conflict, MDR relationship, MDS source coverage and bounded negative, MDT timeline event) and put the case element ids this row would support in element_ids, semicolon separated, from: C08.E5, C06.E3. Leave any column that does not apply to a row empty rather than guessing.

Task A — For each role METR holds, record who selected or endorsed it and by what documented process, with the document and date.
Task B — Record the alternatives named in the same document or process, without ranking them.
Task C — Record any published criteria for selection and whether independence is among them, quoted verbatim.
Task D — Record where no selection process is published, naming the documents checked.
