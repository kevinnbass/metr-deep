<!-- casework-lane {"claim_id":"C01","elements":["C01.E4","C02.E2","C06.E1"],"materializer":2,"outputs":["research/grok-out/MD50-audit-money.csv"],"pack":"metr_deep","plan_lane":"MD50","plan_sha256":"7ff1e5a3d68d5eadfdb76c0ff002060e21ce4adbff1fa3c5c6ef52d5cad0f9e2"} -->

# MD50 — Money and provenance adversarial audit
...
Scope: Money and provenance adversarial audit.
Primary routes: promoted rows and their primaries.
...
Row typing: put the target table prefix in target_prefix on every row (MDE entity, MDF funding event, MDI in-kind or access, MDP provenance proposition, MDQ project conflict, MDR relationship, MDS source coverage and bounded negative, MDT timeline event) and put the case element ids this row would support in element_ids, semicolon separated, from: C01.E4, C02.E2, C06.E1. Leave any column that does not apply to a row empty rather than guessing.

Task A — Check every promoted money row against its primary and assign a verdict, listing every row whose amount, date, entity or money type does not match.
Task B — Attack the reconciliation: find every amount that could be double counted, every period mismatch and every money type used as evidence of another.
Task C — Attack every bounded negative: find any that is unbounded in fact, or whose named source would not have carried the record anyway.
Task D — State whether the exhaustion gate can be approved on the money side, and list every correction required first.
