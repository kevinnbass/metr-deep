<!-- casework-lane {"claim_id":"C04","elements":["C04.E1"],"materializer":2,"outputs":["research/grok-out/MD26-tokens-credits-compute.csv"],"pack":"metr_deep","plan_lane":"MD26","plan_sha256":"7ff1e5a3d68d5eadfdb76c0ff002060e21ce4adbff1fa3c5c6ef52d5cad0f9e2"} -->

# MD26 — Tokens, credits, compute and engineering help by lab and project
...
Scope: Tokens, credits, compute and engineering help by lab and project.
Primary routes: METR reports and acknowledgments; lab statements and system cards.
...
Row typing: put the target table prefix in target_prefix on every row (MDE entity, MDF funding event, MDI in-kind or access, MDP provenance proposition, MDQ project conflict, MDR relationship, MDS source coverage and bounded negative, MDT timeline event) and put the case element ids this row would support in element_ids, semicolon separated, from: C04.E1. Leave any column that does not apply to a row empty rather than guessing.

Task A — For every METR assessment or report naming a lab, record the acknowledgment of tokens, credits, compute or engineering support verbatim, with the report, its date and the provider named.
Task B — Record a quantity or estimated value only where a source states one, with the source's own wording; leave the value empty rather than estimating.
Task C — Record each lab's own statement about resources given to METR from the lab's documents, and mark any difference from METR's wording.
Task D — Record explicitly that free resources are not cash, and never carry an in-kind figure into a money total.
...
Output `research/grok-out/MD26-tokens-credits-compute.csv` with columns: `row,task,target_prefix,element_ids,subject,entity_type,aliases,identifier,from_entity,intermediary,to_entity,relationship_type,start_date,end_date,ledger,money_type,amount_usd,currency,date,date_precision,period,payment_status,purpose_restriction,project,provider,in_kind_type,quantity_or_value,terms,policy_version,disclosure_recusal,provider_authority,personnel_conflicts,edge_type,documented_transaction,source_class,query_or_endpoint,url,quote_300,checked_utc,result,result_count,next_document,limitation,note`.

Standard goal:

```text
/goal Run research/grok-briefs/MD26-tokens-credits-compute.md exactly as written; write only research/grok-out/MD26-tokens-credits-compute.csv and research/primary/MD26-tokens-credits-compute/
```
