<!-- casework-lane {"claim_id":"C01","elements":["C01.E5","C02.E3","C07.E3"],"materializer":2,"outputs":["research/grok-out/MD45-calendar-monitor.csv"],"pack":"metr_deep","plan_lane":"MD45","plan_sha256":"7ff1e5a3d68d5eadfdb76c0ff002060e21ce4adbff1fa3c5c6ef52d5cad0f9e2"} -->

# MD45 — Calendar monitor for filings and reports
...
Scope: Calendar monitor for filings and reports.
Primary routes: official release points only.
...
Row typing: put the target table prefix in target_prefix on every row (MDE entity, MDF funding event, MDI in-kind or access, MDP provenance proposition, MDQ project conflict, MDR relationship, MDS source coverage and bounded negative, MDT timeline event) and put the case element ids this row would support in element_ids, semicolon separated, from: C01.E5, C02.E3, C07.E3. Leave any column that does not apply to a row empty rather than guessing.

Task A — Build one calendar row per expected document: the document, the issuer, the earliest expected date, the official release point and the claim elements it would bear on.
Task B — For each row, record the check performed now, the result and the UTC time.
Task C — Record any document that has become available since the last check and route it to the lane that owns it.
Task D — Record explicitly that a document not yet due is a calendar closer, not a negative finding.
