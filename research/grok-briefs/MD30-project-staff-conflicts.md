<!-- casework-lane {"claim_id":"C08","elements":["C08.E2","C05.E1"],"materializer":2,"outputs":["research/grok-out/MD30-project-staff-conflicts.csv"],"pack":"metr_deep","plan_lane":"MD30","plan_sha256":"7ff1e5a3d68d5eadfdb76c0ff002060e21ce4adbff1fa3c5c6ef52d5cad0f9e2"} -->

# MD30 — Project staff conflicts, disclosures and recusals
...
Scope: Project staff conflicts, disclosures and recusals.
Primary routes: report disclosures; public bios; the conflict policy; public statements only.
...
Row typing: put the target table prefix in target_prefix on every row (MDE entity, MDF funding event, MDI in-kind or access, MDP provenance proposition, MDQ project conflict, MDR relationship, MDS source coverage and bounded negative, MDT timeline event) and put the case element ids this row would support in element_ids, semicolon separated, from: C08.E2, C05.E1. Leave any column that does not apply to a row empty rather than guessing.

Task A — For each material project, list the staff the documents name and record each person's disclosed conflicts exactly as the document states them.
Task B — Separate, for each person, direct equity, recent lab employment, simultaneous lab work, a stated close personal relationship, a board role and mere ecosystem adjacency; never merge two categories.
Task C — Record only what the person or the organisation has made public; record no inference about a private relationship and no unpublished personal information.
Task D — Record each disclosed recusal and each case where a document states that no recusal was applied.
...
Output `research/grok-out/MD30-project-staff-conflicts.csv` with columns: `row,task,target_prefix,element_ids,subject,entity_type,aliases,identifier,from_entity,intermediary,to_entity,relationship_type,start_date,end_date,ledger,money_type,amount_usd,currency,date,date_precision,period,payment_status,purpose_restriction,project,provider,in_kind_type,quantity_or_value,terms,policy_version,disclosure_recusal,provider_authority,personnel_conflicts,edge_type,documented_transaction,source_class,query_or_endpoint,url,quote_300,checked_utc,result,result_count,next_document,limitation,note`.

Standard goal:

```text
/goal Run research/grok-briefs/MD30-project-staff-conflicts.md exactly as written; write only research/grok-out/MD30-project-staff-conflicts.csv and research/primary/MD30-project-staff-conflicts/
```
