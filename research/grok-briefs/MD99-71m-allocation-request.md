<!-- casework-lane {"claim_id":"C01","elements":["C01.E4","C02.E2"],"materializer":2,"outputs":["research/grok-out/MD99-71m-allocation-request.csv"],"pack":"metr_deep","plan_lane":"MD99","plan_sha256":"7ff1e5a3d68d5eadfdb76c0ff002060e21ce4adbff1fa3c5c6ef52d5cad0f9e2"} -->

# MD99 — Single consolidated request for the unresolved allocation

Plan directive:

Slice S6, wave 6. This lane belongs to /mnt/f/projects/anthropic/metr_deep/PLAN.md; its id, slug and scope are frozen and are never renumbered or repurposed.

Scope: Single consolidated request for the unresolved allocation.
Primary routes: the exhaustion gate and the residual unknown matrix only.
...
Row typing: put the target table prefix in target_prefix on every row (MDE entity, MDF funding event, MDI in-kind or access, MDP provenance proposition, MDQ project conflict, MDR relationship, MDS source coverage and bounded negative, MDT timeline event) and put the case element ids this row would support in element_ids, semicolon separated, from: C01.E4, C02.E2. Leave any column that does not apply to a row empty rather than guessing.

Task A — Read the exhaustion gate and list the residual cells it enumerates; ask about nothing already established.
Task B — Draft one short request asking, for the unresolved portion only, the donor or vehicle, the committed amount, the commitment date, the cash paid to date, the restriction or purpose, and whether in-kind support is counted inside the total.
Task C — Attach or link the public-source reconciliation so the recipient can correct specific cells rather than answer a theory; include no allegation and no compound rhetorical question.
Task D — Save the draft with approved_for_send false and record the state as awaiting the user's authority. Send nothing.
...
Output `research/grok-out/MD99-71m-allocation-request.csv` with columns: `row,task,target_prefix,element_ids,subject,entity_type,aliases,identifier,from_entity,intermediary,to_entity,relationship_type,start_date,end_date,ledger,money_type,amount_usd,currency,date,date_precision,period,payment_status,purpose_restriction,project,provider,in_kind_type,quantity_or_value,terms,policy_version,disclosure_recusal,provider_authority,personnel_conflicts,edge_type,documented_transaction,source_class,query_or_endpoint,url,quote_300,checked_utc,result,result_count,next_document,limitation,note`.

Standard goal:

```text
/goal Run research/grok-briefs/MD99-71m-allocation-request.md exactly as written; write only research/grok-out/MD99-71m-allocation-request.csv and research/primary/MD99-71m-allocation-request/
```
