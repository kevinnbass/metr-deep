<!-- casework-lane {"claim_id":"C07","elements":["C07.E3","C05.E1"],"materializer":2,"outputs":["research/grok-out/MD42-anthropic-securities-filings.csv"],"pack":"metr_deep","plan_lane":"MD42","plan_sha256":"7ff1e5a3d68d5eadfdb76c0ff002060e21ce4adbff1fa3c5c6ef52d5cad0f9e2"} -->

# MD42 — Anthropic securities filings and holder disclosure
...
Scope: Anthropic securities filings and holder disclosure.
Primary routes: EDGAR full-text and issuer search; issuer charter and bylaws where public.
...
Row typing: put the target table prefix in target_prefix on every row (MDE entity, MDF funding event, MDI in-kind or access, MDP provenance proposition, MDQ project conflict, MDR relationship, MDS source coverage and bounded negative, MDT timeline event) and put the case element ids this row would support in element_ids, semicolon separated, from: C07.E3, C05.E1. Leave any column that does not apply to a row empty rather than guessing.

Task A — Search EDGAR by issuer, by central index key and by full text for any Anthropic registration statement or holder disclosure, and record the query, the result count and the UTC check time.
Task B — Record what a registration statement would and would not disclose about holders, citing the rule or instruction, so that absence is correctly bounded.
Task C — Record any public transfer restriction in the issuer's charter or bylaws that bears on how a holding could be held or moved, quoted verbatim.
Task D — Record the filing as a calendar closer with its next check date if it does not yet exist.
...
Output `research/grok-out/MD42-anthropic-securities-filings.csv` with columns: `row,task,target_prefix,element_ids,subject,entity_type,aliases,identifier,from_entity,intermediary,to_entity,relationship_type,start_date,end_date,ledger,money_type,amount_usd,currency,date,date_precision,period,payment_status,purpose_restriction,project,provider,in_kind_type,quantity_or_value,terms,policy_version,disclosure_recusal,provider_authority,personnel_conflicts,edge_type,documented_transaction,source_class,query_or_endpoint,url,quote_300,checked_utc,result,result_count,next_document,limitation,note`.

Standard goal:

```text
/goal Run research/grok-briefs/MD42-anthropic-securities-filings.md exactly as written; write only research/grok-out/MD42-anthropic-securities-filings.csv and research/primary/MD42-anthropic-securities-filings/
```
