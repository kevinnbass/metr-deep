<!-- casework-lane {"claim_id":"C03","elements":["C03.E1","C03.E2","C03.E3"],"materializer":2,"outputs":["research/grok-out/MD32-lab-money-negative-sweep.csv"],"pack":"metr_deep","plan_lane":"MD32","plan_sha256":"7ff1e5a3d68d5eadfdb76c0ff002060e21ce4adbff1fa3c5c6ef52d5cad0f9e2"} -->

# MD32 — Direct lab payment and employee-directed donation negative sweep
...
Scope: Direct lab payment and employee-directed donation negative sweep.
Primary routes: lab disclosures; METR filings and pages; donor-advised sponsor rules.
...
Row typing: put the target table prefix in target_prefix on every row (MDE entity, MDF funding event, MDI in-kind or access, MDP provenance proposition, MDQ project conflict, MDR relationship, MDS source coverage and bounded negative, MDT timeline event) and put the case element ids this row would support in element_ids, semicolon separated, from: C03.E1, C03.E2, C03.E3. Leave any column that does not apply to a row empty rather than guessing.

Task A — Record METR's stated rule on funding from AI companies and on employee-directed donations, in each dated version, quoted verbatim.
Task B — Search each named frontier lab's own disclosures, grant lists and filings for any payment, contract, sponsorship or donation to METR, and record each source checked with its result.
Task C — Search METR's own filings and pages for any lab-sourced revenue line, and record the fields checked.
Task D — Record the exact point at which public verification stops for a donation routed through a donor-advised account, naming the schedule that would show it if sponsors disclosed principals.
...
Output `research/grok-out/MD32-lab-money-negative-sweep.csv` with columns: `row,task,target_prefix,element_ids,subject,entity_type,aliases,identifier,from_entity,intermediary,to_entity,relationship_type,start_date,end_date,ledger,money_type,amount_usd,currency,date,date_precision,period,payment_status,purpose_restriction,project,provider,in_kind_type,quantity_or_value,terms,policy_version,disclosure_recusal,provider_authority,personnel_conflicts,edge_type,documented_transaction,source_class,query_or_endpoint,url,quote_300,checked_utc,result,result_count,next_document,limitation,note`.

Standard goal:

```text
/goal Run research/grok-briefs/MD32-lab-money-negative-sweep.md exactly as written; write only research/grok-out/MD32-lab-money-negative-sweep.csv and research/primary/MD32-lab-money-negative-sweep/
```
