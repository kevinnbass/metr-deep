<!-- casework-lane {"claim_id":"C04","elements":["C04.E2","C04.E3","C08.E3"],"materializer":2,"outputs":["research/grok-out/MD27-access-and-project-terms.csv"],"pack":"metr_deep","plan_lane":"MD27","plan_sha256":"7ff1e5a3d68d5eadfdb76c0ff002060e21ce4adbff1fa3c5c6ef52d5cad0f9e2"} -->

# MD27 — Model access, safe harbor, publication, redaction and exit rights
...
Scope: Model access, safe harbor, publication, redaction and exit rights.
Primary routes: assessment reports; published agreements; policies.
...
Row typing: put the target table prefix in target_prefix on every row (MDE entity, MDF funding event, MDI in-kind or access, MDP provenance proposition, MDQ project conflict, MDR relationship, MDS source coverage and bounded negative, MDT timeline event) and put the case element ids this row would support in element_ids, semicolon separated, from: C04.E2, C04.E3, C08.E3. Leave any column that does not apply to a row empty rather than guessing.

Task A — For each engagement, record the model access granted as the documents describe it: level of access, transcripts, sampling, internal materials, and the dates access applied.
Task B — Record the stated safe harbor, non-disclosure, publication and pre-publication review terms, quoted verbatim from the document that states them.
Task C — Record whether the provider holds any redaction right or any authority to end the engagement before publication, and the document that grants or withholds it.
Task D — Where a term is not disclosed, record that explicitly with the exact documents checked, rather than inferring it from another engagement.
...
Output `research/grok-out/MD27-access-and-project-terms.csv` with columns: `row,task,target_prefix,element_ids,subject,entity_type,aliases,identifier,from_entity,intermediary,to_entity,relationship_type,start_date,end_date,ledger,money_type,amount_usd,currency,date,date_precision,period,payment_status,purpose_restriction,project,provider,in_kind_type,quantity_or_value,terms,policy_version,disclosure_recusal,provider_authority,personnel_conflicts,edge_type,documented_transaction,source_class,query_or_endpoint,url,quote_300,checked_utc,result,result_count,next_document,limitation,note`.

Standard goal:

```text
/goal Run research/grok-briefs/MD27-access-and-project-terms.md exactly as written; write only research/grok-out/MD27-access-and-project-terms.csv and research/primary/MD27-access-and-project-terms/
```
