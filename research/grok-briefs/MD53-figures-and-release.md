<!-- casework-lane {"claim_id":"C10","elements":["C10.E1","C10.E2","C10.E3"],"materializer":2,"outputs":["research/grok-out/MD53-figures-and-release.csv"],"pack":"metr_deep","plan_lane":"MD53","plan_sha256":"7ff1e5a3d68d5eadfdb76c0ff002060e21ce4adbff1fa3c5c6ef52d5cad0f9e2"} -->

# MD53 — Figures, bibliography, verdicts and release lint
...
Scope: Figures, bibliography, verdicts and release lint.
Primary routes: promoted rows only; local artifacts only.
...
Task A — Render only figures whose claims are ready, and record for each the rows it cites and the unknown block it shows.
Task B — Build the evidence bibliography so that every number on a figure resolves to a promoted row id.
Task C — Write the verdict summary: the strongest supported thesis, the strongest unsupported thesis, the posted and current version distinction, and every outstanding document.
Task D — Run the release lint and record its result; publish nothing and push nothing.
...
```text
/goal Run research/grok-briefs/MD53-figures-and-release.md exactly as written; write only research/grok-out/MD53-figures-and-release.csv and research/primary/MD53-figures-and-release/
```
